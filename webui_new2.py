...
def generate_clicked(*args):
    ...
    if state == 1:
        yield gr.update(visible=False), \
            ...
            gr.update(visible=True, value='当前其他用户正在生成图片，请稍后重试.')
        ...
        # cur2.execute("SELECT * FROM busy WHERE isbusy = 0")
        # busy = cur2.fetchone()
        if cur2.execute("UPDATE busy SET isbusy = 1 WHERE isbusy = 0") != 0:
            ...
            pool.release(connection2)
            state = 0
            yield gr.update(visible=True, value=modules.html.make_progress_html(0, "当前系统繁忙，请稍后重试")), \
        ...
        try:
            ...
            args1 = list(args)
            # 如果prompt不含英文
            if not re.search('[a-zA-Z]', args1[0]) and args1[0] != "":
                url1 = "http://172.17.240.85:7863/v1/CNPrompt2ENPrompt"
                payload1 = {
                    "prompt": args1[0],
                    "task_id": "1"
                }
                ...
                response1 = requests.post(url1, headers=headers, json=payload1)
                args1[0] = response1.json()

            if not re.search('[a-zA-Z]', args1[1]) and args1[1] != "":
                url1 = "http://172.17.240.85:7863/v1/CNPrompt2ENPrompt"
                payload1 = {
                    "prompt": args1[1],
                    "task_id": "1"
            ...
            response = requests.post(url, headers=headers, json=payload, stream=True)  # 请求接口，很长的任务
            ...
            for line in response.iter_lines(decode_unicode=True):
                if line:
                    ...
                    if result[0] == "error":
                        ...
                        yield gr.update(visible=False), \
                            gr.update(visible=False), \
                            gr.update(visible=True, value=(base64_to_numpy(result[1])))
                        break
                    elif result[0] == "preview":
                        percentage, title, image = result[1]
            ...
            cur.execute("UPDATE busy SET isbusy = 0 WHERE isbusy = 1")
            connection.commit()
            ...
            with gr.Tab(label='高级设置'):
                ...
                with gr.Column(visible=False) as dev_tools:
                    ...
                    with gr.Tab(label='FreeU'):
                        ...
                        freeu_ctrls = [freeu_enabled.value, freeu_b1.value, freeu_b2.value, freeu_s1.value,
                                       freeu_s2.value]
                ...
                # adps = [adm_scaler_positive.value, adm_scaler_negative.value, adm_scaler_end.value, adaptive_cfg.value,
                #         sampler_name.value, scheduler_name.value, overwrite_step.value, overwrite_switch.value,
                #         overwrite_width.value, overwrite_height.value, overwrite_vary_strength.value,
                #         overwrite_upscale_strength.value, mixing_image_prompt_and_vary_upscale.value,
                #         mixing_image_prompt_and_inpaint.value, debugging_cn_preprocessor.value,
                #         controlnet_softness.value, canny_low_threshold.value, canny_high_threshold.value,
                #         inpaint_engine.value, refiner_swap_method.value]
                # adps += freeu_ctrls
